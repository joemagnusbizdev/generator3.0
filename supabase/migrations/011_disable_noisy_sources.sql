-- Migration: Disable noisy/news sources provided by user
-- Safely backs up rows to 'sources_disabled_backup' then sets enabled = false
BEGIN;

-- Create a backup table (structure copy) if not exists
CREATE TABLE IF NOT EXISTS public.sources_disabled_backup (LIKE public.sources INCLUDING ALL);

-- Insert the rows we're about to disable into the backup table
INSERT INTO public.sources_disabled_backup
...
-- Disable the sources so they're no longer scoured; preserves history and references
...
-- Verification notes:
-- After applying, run: SELECT id, name, url, enabled FROM public.sources WHERE url IN (...);
