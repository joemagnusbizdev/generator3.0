...
export default function TrendsView({
  ...
  const deleteAlertFromTrend = async (alertId: string, trendId: string) => {
    if (!accessToken) return;
                    ...
                    >
                      <div className="flex items-start justify-between">
                        ...
                        <div className="ml-3 flex items-center gap-2">
                          ...
                          <button
                            onClick={() => deleteAlertFromTrend(alert.id, t.id)}
                            disabled={deletingAlertId === alert.id}
                            className="px-2 py-1 text-xs rounded hover:opacity-90 disabled:opacity-50 whitespace-nowrap"
                            style={{ 
                              backgroundColor: MAGNUS_COLORS.critical, 
                              color: "white",
                          ...
                          >
                            {deletingAlertId === alert.id ? "Deleting..." : "🗑"}
                          </button>
                        </div>
                      </div>
                    </div>
