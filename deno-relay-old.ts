...
async function loadProjectContext(): Promise<string> {
  ...
  try {
    ...
    context += "\n[GÜán+Å Project context loading failed, using fallback]\n";
  }
...
async function callClaude(userMessage: string, chatId: number): Promise<string> {
  if (!ANTHROPIC_API_KEY) {
    return "G¥î Claude API key not configured in Deno Deploy environment variables";
  }
...
When users ask questions about the project, reference the context above. When they ask for code changes, show a preview first.`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return `G¥î Claude error: ${error.substring(0, 100)}`;
    }
    ...
    return `GÜán+Å Error: ${error instanceof Error ? error.message : "Unknown error"}`;
  }
}
...
async function handleMessage(chatId: number, text: string): Promise<string> {
  ...
  // Quick commands
  if (lower === "/help") {
    return `=ƒñû *Claude Project Bot*

=ƒô¥ **Commands:**
GÇó \`/read [path]\` - Read file from GitHub
GÇó \`/status\` - Check project status
GÇó \`/help\` - Show this message
...
_All requests processed by Claude AI_`;
  ...
  if (lower.startsWith("/read ")) {
    ...
    if (!GITHUB_TOKEN) {
      return "G¥î GitHub token not configured";
    }
